...
// ==================================
{
  ...
  // --- ★★★ 初回のみ自動で開くためのロジック ★★★ ---
  // セッションストレージを使って、ブラウザを閉じない限りは「表示済み」と記憶する
  let hasBeenShown = sessionStorage.getItem('greetingModalShown');

  // スクロールして要素が見えたかを監視するIntersection Observer
  const observer = new IntersectionObserver((entries) => {
    entries.forEach(entry => {
      // トリガー要素が画面内に入り、かつ、まだ一度も表示されていない場合
      if (entry.isIntersecting && !hasBeenShown) {
        
        // 0.5秒のディレイを置いてモーダルを自動で開く
        ...
        // 一度実行したら、このスクロール監視は不要なので停止する
        observer.unobserve(entry.target);
      }
    });
  });

  // GREETINGボタンのセクションの監視を開始
